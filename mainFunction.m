function [data,p] = mainFunction( Nshots, Ntotal, delayTimes )
...
for d=1:numel(delayTimes)
    
    %% Prepare DG535
    
    % Establish connection to DG535
    [~,objDelay] = DG535.connect();
    % Set delay
    invoke( objDelay, 'delayTime',2,1,delayTimes(d) );
    % Display information
    fprintf( '(%g/%g) - Preparing measurement...\n\tDelay: %g s\n', d,numel(delayTimes),delayTimes(d) );
    ...
    % Save data
    prefixStr = sprintf( '%g_%g_', d, delayTimes(d)*1e12 );
    SR430.saveData( data(:,:,3), ScalerSettings, 0, 0, [PathName, prefixStr, 'so_', FileName] )
    SR430.saveData( data(:,:,2), ScalerSettings, 0, 0, [PathName, prefixStr, 'sc_', FileName] )
    SR430.saveData( data(:,:,1), ScalerSettings, 0, 0, [PathName, prefixStr, 'bg_', FileName] )
    ...
    extensionStr = sprintf('_expData_%g_%g.mat', d, delayTimes(d)*1e12 );
    save( [PathName, FileName, extensionStr ])
